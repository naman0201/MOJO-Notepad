...
class Notepad:
    def __init__(self):
        ...
        self.t1 = Thread(target=self.cal_time, args=(start_time,))
        self.t1.setDaemon(True)
        self.t1.start()
        self.root.protocol("WM_DELETE_WINDOW", self.exit_func)

    def cal_time(self, start_time):
        curr_time = time.time()
        time_diff = math.floor(curr_time - start_time)
        self.time_label[
            'text'] = f"Time-Elapsed:- {time_diff // 3600}:{(time_diff % 3600) // 60}:{(time_diff % 3600) % 60}"
        time.sleep(1)
        self.cal_time(start_time)
    ...
    def set_icons(self):
        ...
        self.locked_icon = tk.PhotoImage(file='icons/locked.png')
    ...
    def set_tool_bar(self):
        ...
        self.font_size['values'] = tuple(range(8, 81, 2))
    ...
    def set_tool_bar_event_bindings(self):
        ...
        self.mic_btn.configure(command=self.say_something)

    def set_status_bar(self):
        self.status_bar = ttk.Label(self.root,
                                    text=f"Status Bar\tDate: {date.today().strftime('%d-%m-%Y')}")
        self.time_label = tk.Label(self.root,
                                   text=f"Time-Elapsed:- {0}:{0}:{0}")
    ...
    def set_file_sub_menu(self):
        ...
        self.p_file.add_command(label='Secure Files', image=self.locked_icon, compound=tk.LEFT,
                                accelerator='Ctrl+P',
    ...
    def exit_func(self, event=None):
        try:
            if self.text_changed:
                answer = messagebox.askyesnocancel("Confirmation", message="Do you want to save the file?")
                if answer:
    ...
    def open_file(self, event=None):
        try:
            ...
            messagebox.showerror("Wrong File Name", "File doesn't Exist!")
        except:
            print(traceback.format_exc())
    ...
    def changed(self, event=None):
        if self.text_editor.edit_modified():
            ...
            self.status_bar.config(
                text=f"\tWords:{words}\tCharacters:{characters}\tDate: {date.today().strftime('%d-%m-%Y')}")
    ...
    def find_func(self, event=None):
        ...
        self.find_dialogue.geometry('400x250+500+200')
    ...
    def say_something(self):
        try:
            ...
            print("You said: " + self.takeAudio)
            if self.takeAudio == "":
                messagebox.showerror("say again", "speech not recognized")
            elif self.takeAudio.lower() in 'hey mojo open file':
                self.open_file()
            elif self.takeAudio.lower() in 'hey mojo save file':
                self.save_file()
            elif self.takeAudio.lower() in 'hey mojo open new file':
                self.new_file()
            elif self.takeAudio.lower() in 'hey mojo close app':
                self.exit_func()
            elif self.takeAudio.lower() in 'hey mojo bold text':
                self.change_bold()
            elif self.takeAudio.lower() in 'hey mojo italic text':
                self.change_italic()
            elif self.takeAudio.lower() in 'hey mojo underline text':
                self.change_underline()
            elif self.takeAudio.lower() in 'hey mojo change colour':
                self.change_font_color()
            elif self.takeAudio.lower() in 'hey mojo open colour palette':
                self.change_font_color()
            elif self.takeAudio.lower() in 'hey mojo search':
                self.find_func()
            elif self.takeAudio.lower() in 'hey mojo hide statusbar':
                self.hide_statusbar()
            elif self.takeAudio.lower() in 'hey mojo hide toolbar':
                self.hide_toolbar()
            ...
            messagebox.showerror("No translatable text", "Speech unrecognizable")
    ...
    def change_theme(self):
        ...
        fg_color, bg_color = self.color_dict[theme_name]
    ...
    def secure_file(self, event=None):
        if self.db_status:
            ...
            self.file_dialogue.geometry('400x400+500+200')
            ...
            self.file_remove_button = ttk.Button(self.file_frame, text='Remove File',
                                                 command=self.remove_secure_file)
    ...
    def open_secure_file(self):
        try:
            ...
            if len(self.sel_file_index_tuple) == 0:
                ...
                self.file_name = self.full_file_name[:self.full_file_name.find("[")].strip()
    ...
    def get_file_pwd(self):
        return simpledialog.askstring("Password", "Enter File Password:", show="*")
